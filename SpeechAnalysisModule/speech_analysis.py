...
class SpeechAnalysisModule:
    def __init__(self, asr_model_path = "vosk-model-small-en-us-0.15", vad_model_dir = "silero_models", keyword_file="list_of_words.txt", rnnoise_path="linux-rnnoise/rnnoise_mono.lv2/librnnoise_mono.so", noise_filter=False):
        ...
        #                       source="local",
        #                       force_reload=True)
        self.vad_model = OnnxWrapper(vad_model_dir + "/src/silero_vad/data/silero_vad_half.onnx")
        self.vad_model = load_silero_vad(onnx=True)
        print("Loaded", self.vad_model)
    ...
    def listen(self):
        ...
        while True: 
            ...
            if not is_listening:
                ...
                # < add following
                timings.append( time.time() - vad_start )
                if len(timings) > 20:
                    timings.pop(0)
                print("VAD takes", sum(timings)/ len(timings),"seconds on avg") # add
                # end add >
